...
performance and robust identification of symmetry. Some of these clustering methods
...
to which each contour belongs can be determined. The region of the domain
...
to obtain the cumulative similarity score between $p$ and $q$. Analogous procedure
...
The simple strategy we adopt is to ensure that none of the contours merge by
...
with respect to merging of contours is captured using a \emph{stabilisation parameter} $\alpha$.
After discarding the saddles of all arcs that are considered to be noise, for a join saddle $s$,
where contours merge into one, we select an isovalue $w = f(s)-\alpha$ if there are no other 
saddles in the interval $[w,f(s)]$. Among the set of isovalues thus generated,
...
Observe that the supplemental material of the extremum graph based symmetry detection method shows 
