...
%	{
		...
		{
			\centering
			\includegraphics[scale=.28]{figures/1897-full.png}
		}
		\end{minipage}
%	}
	\hspace{0.02cm}
...
%	{
		\begin{minipage}[c][1\width]{0.32\textwidth}
		{
			\centering
			\hspace{0.2cm}
			\includegraphics[scale=.26]{inkscape/descspace.pdf}
		}
		\end{minipage}
%	}	
%	\subfigure[]
%	{
		...
		{
			\centering
			\includegraphics[scale=.28]{figures/1897-sym.png}
		}
		\end{minipage}
%	}
...
symmetry plays an important role in studying the underlying scientific phenomenon. 
...
domain. Symmetry in shapes is associated with a group structure on
...
for studying the underlying physical phenomenon. Hence, symmetry identification methods that are based on 
...
Fig.~\ref{teaser} illustrates our approach on a 3D cryo-EM image of AMP-activated kinase (EMDB-1897) 
with three-fold rotational symmetry. Our method identifies symmetric regions of different
...
provides a lot of flexibility in analyzing similarity of scalar fields
...
that employs elegant optimizations by incorporating topological information 
...
the sample points. Inspecting only the sample points
...
detection~\cite{BrucknerM10,haidacher11}. They use mutual information between distance 
...
similar isosurfaces both within and across datasets. While this method is related since
it is also based on isosurface similarity, the goal here is to select a subset of representative
and possibly important isovalues. The distance transform is used to identify redundant isovalues 
corresponding to families of isosurfaces that form an onion-peel like layered arrangement. 
The goal of our method, on the other hand, is to locate regions that are similar and hence
we compute similarity between contours and not isosurfaces. While the isosurface similarity map based
method is limited to analyzing similarity between pairs of datasets, the descriptor space can 
be used to analyze multiple datasets simultaneously. Similarity between different scalar 
...
data structure that captures these changes in the connectivity of the level sets, see Fig.~\ref{ct}.
...
\emph{descriptor} and the high-dimensional space is called the \emph{descriptor space}, see Fig.~\ref{ctmap}. 
...
\centering
{
	\includegraphics[scale=0.14]{inkscape/ctrmap.pdf}
...
\subsection{Overview}
Fig.~\ref{pipeline} illustrates the main steps of our algorithm.
...
\begin{figure*}[t]
	\centering
		\includegraphics[scale=0.12]{inkscape/pipe.pdf}
...
largest contour associated with the arc is below a user defined \emph{noise threshold} $\delta$. 
Assuming that the scalar field is uniformly sampled, the volume of a
...
%		\begin{minipage}[c][1\width]{0.3\linewidth}
		{
			...
			%\includegraphics[scale=.08]{figures/ctrs-before-merge.png}
		}
%		\end{minipage}
	}
%	\subfigure[]
...
%		\begin{minipage}[c][1\width]{0.3\linewidth}
		{
			\centering
			$\vcenter{\hbox{\includegraphics[scale=.25]{inkscape/ctmerge.pdf}}}$
			%\includegraphics[scale=.2]{inkscape/ctmerge.pdf}
		}
%		\end{minipage}
	}
%	\subfigure[]
...
%		\begin{minipage}[c][1\width]{0.3\linewidth}
		{
			...
			%\includegraphics[scale=.08]{figures/ctrs-after-merge.png}
		}
%		\end{minipage}
...
see Fig.~\ref{ct-merge}. Recall that a cluster in the descriptor space represents a
...
see Fig.~\ref{bip}. An edge between ${c_i}^p$ and ${c_j}^q$ is weighted
...
at which the contours from symmetric regions merge and split may not be consistent. Fig.~\ref{alpha0pt2}
...
We introduce a \emph{stabilization parameter} $\alpha$ that determines
...
analysis required by an application. A function $\phi$ which satisfies the equation 
...
employ a simple scheme based on nearest neighbor search for clustering.
...
space that lie within a sphere of radius $\epsilon$ centered at $p_c$,
...
the level set of $c$. 
The ideal metric for computing
...
\begin{minipage}{\textwidth}
	...
	\subfigure[]
	{
		\includegraphics[scale=0.1]{figures/isa12.png}
		\label{conisa12}
	}
		\hspace{40pt}
	\subfigure[]
	{
		\includegraphics[scale=0.1]{figures/isa24.png}
		\label{conisa24}
	}
		\hspace{40pt}
	\subfigure[]
	{
		\includegraphics[scale=0.1]{figures/isa36.png}
		\label{conisa36}
	}
		\hspace{40pt}
	\subfigure[]
	{
		\includegraphics[scale=0.1]{figures/isa48.png}
		\label{conisa48}
	}
		\hspace{30pt}
\end{minipage}
...
on a 500$\times$500$\times$100 grid over 48 time steps. Fig.~\ref{volisa1} shows a volume rendering
...
Fig.~\ref{gaus}, as the query object. We generate a scale invariant version of the Laplace-Beltrami
...
volume and compute their scale invariant descriptors. Finally, we locate the nearest neighbor of the descriptor 
representing the query object in the descriptor space. We identify the contour
shown in Fig.~\ref{conisa1} through this search. Note that the size and the exact shape of the
...
We use the contour shown in Fig.~\ref{conisa1} from the first time step as the query object to search through 
the other time steps. The contours detected as a result of executing the query on four different time 
steps, 12, 24, 36, and 48, are shown in Fig.~\ref{conisa12}-~\ref{conisa48}. Observe that
...
of the mouth of the contour is closed in Fig.~\ref{conisa48} as the hurricane makes landfall.
The query is successful even with these challenges and emphasizes the robustness of our method.
Similar to Fig.~\ref{detisa1}, the volumetric region of the domain can be extracted in 
...
physical phenomenon. For example, asymmetry between 
...
The distance computed at each vertex is stored as a scalar field of 
...
Fig.~\ref{asym} shows a cryo-EM dataset in which the top and bottom regions are symmetric. The procedure
...
\end{minipage}
	\caption{\label{res}Comparison with topological methods for symmetry detection.  
		...
		of the electron density of a hemoglobin molecule (PDB-ID 1HGA) that exhibit 2-fold rotational symmetry. 
...
The segmentation of the domain utilized by our algorithm is
...
Our method combines the advantage of these two methods. The first row in Fig.~\ref{res} shows the result of our
...
set is non-trivial. Fig.~\ref{kneevol} shows a volume rendering of a CT scan of a pair of knees. Observe the
...
in orange, blue, and pink in Fig.~\ref{kneectrssym}. An inspection of these contours show that the dark regions of the bone in 
...
the global maximum in Fig.~\ref{kneectrsflat} shows that the global maximum is degenerate
and forms a flat region. 
Fig.~\ref{1hga-full} shows an electron density field
derived from the atomic coordinates of a hemoglobin molecule using EMAN software~\cite{ludtke1999eman}.
Our method identifies two symmetric regions, shown in blue and pink in Fig.~\ref{1hga-sym},
despite these two regions being in close proximity to each other. The isosurface for an isovalue close to the
global maximum, see Fig.~\ref{1hgaext}, shows that the extrema belonging to the high density regions are not
...
\begin{figure*}[t]
	...
	\caption{\label{alpha}Influence of stabilization parameter, $\alpha$, on symmetry detection. 
...
The value of the stabilization parameter affects the detection of features that are very
transient with respect to the evolution of contours during a level set sweep. Choosing
too small a value for the stabilization parameter may result in only partial detection
...
in Fig.~\ref{1292-full}. In an attempt to capture the symmetry of the small unstable regions within
...
regions generate small-scale contours belonging to the tube-like region, shown
in blue in Fig.~\ref{alpha0pt2}. Due to the instability of the saddles, the contours belonging to
...
to stabilize the branch decomposition representation of the contour tree~\cite{ThomN11}. We plot
the drop in the number of critical points with increasing simplification of the contour tree,
see Fig.~\ref{alphaplot}. Initially, there is a sudden drop due to the unstable critical points, 
...
Fig.~\ref{1292-stable} depicts the 7-fold symmetry of the small blue contours from the top portion of
...
see Fig.~\ref{1292-sym}. This is because these contours belong to longer arcs and are 
insensitive to the exact value of $\alpha$. The inability to detect symmetric
...
$\epsilon$. Nearest neighbor search can be performed efficiently. Therefore, if the first and 
...
Knee 		&$379\times229\times305$ 	&120.4		&94.6		&429.1		&442	&1222491 
\end{tabular}
}
\end{table}
for a dataset.

Fig.~\ref{eps} illustrates the effect of varying $\epsilon$ on a cryo-EM dataset (EMDB-5214).
...
Fig.~\ref{eps0pt1}, are detected. As the value of $\epsilon$ is increased, more approximately
symmetric contours are detected as shown in Fig.~\ref{eps1}-\ref{eps10}. In each figure,
contours belonging to the same cluster are shown with the same color.
...
Fig.~\ref{fuel-full}, this plot is shown in the supplemental material. The plot shows a sudden
drop in the number of arcs initially, similar to the graph used for determining the stabilization
...
4-fold rotational symmetry are identified, see Fig.~\ref{fuel-sym200}. Lowering $\delta$ to 20 vertices
identifies two more smaller symmetric regions as illustrated in Fig.~\ref{fuel-sym20}. Thus, $\delta$
acts as a parameter that controls the size of features analyzed for multiscale symmetry detection.
...
Table~\ref{exp-table} lists the running time of our algorithm on different datasets. The
...
It is beneficial to employ such a criteria that selects a stable contour 
...
\acknowledgments{
	This work was partially supported by the Department of Science and Technology, India, under Grant SR/S3/EECE/0086/2012 and by the Robert Bosch Centre for Cyber Physical Systems, Indian Institute of Science. The second author was supported by a fellowship for experienced researchers from the Alexander von Humboldt Foundation. Hurricane Isabel data was produced by the Weather Research and Forecast (WRF) model, courtesy of NCAR and the U.S. National Science Foundation
	(NSF). Volume rendered images
