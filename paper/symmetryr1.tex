...
\teaser{
	...
	which exhibit 3-fold rotational symmetry. (b)~Contours belonging to symmetric regions are also symmetric. Contours 
	are represented as points in a high dimensional descriptor space (shown in 2D for illustration) using
	a suitable shape descriptor. Since similar contours have similar descriptors, points corresponding to symmetric contours 
	form a cluster in the descriptor space and can be easily identified.  Three such clusters detected are shown in orange, 
	blue, and pink. (c)~Symmetric regions in the volume corresponding to the symmetric contours are extracted and three 
	different symmetric regions are highlighted in orange, blue, and pink corresponding to the three clusters of contours.}
}
...
in higher level information for studying the underlying physical phenomena
obtained through feature extraction methods like segmentation and isosurface extraction. 
Hence, symmetry detection methods based on local information available at the sample points
...
copies are mapped to the same point. We call this high dimensional space
