...
\teaser{
	...
	Three such clusters located are shown in golden, blue, and pink colours. (c)~Three symmetric regions of
	different sizes highlighted in golden, blue, and pink colours demonstrates the multiscale aspect of our method.}
...
\begin{figure*}[t]
	\centering
		\includegraphics[scale=0.15]{inkscape/pipe.pdf}
...
\begin{figure}
	...
	\subfigure[]
	{
			\includegraphics[scale=.12]{figures/ct.png}
	}
...
our method.}
%(i)-(l)~The corresponding volumetric region identified is show in yellow.}
\end{figure*}
...
\begin{minipage}{\textwidth}
	...
	\subfigure[]
	{
		\includegraphics[scale=0.11]{figures/kneectrsflat.png}
		\label{kneectrsflat}
	}
	\subfigure[]
	{
		\includegraphics[scale=0.14]{figures/1hgaext.png}
		\label{1hgaext}
	}
\end{minipage}
	\caption{\label{res}Symmetric regions detected on different datasets. (a)~Volume rendering of a cryo-EM dataset (EMDB-1654),
		with noisy regions depicted in violet. (b)-(e)~Symmetric regions at multiple scales detected by our method.
		The extremum graph based method can detect symmetry only at the largest scale. (f)~Volume rendering of
		a pair of knees. (g)~Bilateral symmetry of the bones detected by our method, shown in orange, blue, and pink, 
		even in the presence of missing regions and noise in the contours of the dataset. (h)~Volume rendering
		of the electron densty of a haemoglobin	molecule (PDB-ID 1HGA) that exhibit 2-fold rotational symmetry. 
	...
	the extrema of the molecule dataset pose challenges in determining the seed set for the extremum graph based method.}
\end{figure*}
\section{Discussion}
In this section, we report the results of our method on different datasets and elaborate on
...
the global maximum in Figure~\ref{kneectrsflat} shows that the global maximum forms a flat region. 
...
symmetrically distributed within the two symmetric regions. In both these cases, selection of a set of extrema as seed set 
is not easy because of the flat regions and the asymmetric distribution of the extrema in the data. 
\begin{figure*}[t]
	\centering
	\subfigure[]
	{
	...
	\subfigure[]
	{
		\includegraphics[scale=0.1]{figures/alphaplot.png}
		\label{alphaplot}
	}
	\caption{\label{alpha}Influence of stabilisation parameter, $\alpha$, on symmetry detection. 
		...
		values generates unstable contours within the tube-like regions indicated by inconsistency
		in the merging of the blue and green contours. (c)~Setting $\alpha$ to 1\% discards the unstable contours
		and the symmetry of the small blue contours generated at a more stable isovalue are identified.
		...
		(e)~The plot of the number of critical points with increasing simplification shows an initial sudden drop
	and we set $\alpha$ to a value immediately after this drop.}
\end{figure*}
\begin{figure*}[t]
	\centering
	\subfigure[]
...
\hline
1HGA	 	& $72^3$ 			& 1.33 		& 7.86 		&  	28.39       	\\%& 14397		639
EMDB-1654 	& $112^3$ 			& 4.64 		& 1.04		&  	8.70      	\\%& 6217		432 
Vortex		& $253^3$			& 82.04	 	& 19.91		&  	163.56		\\
EMDB-1179 	& $255^3$ 			& 67.39		& 10.63		&  	84.91     	\\%& 19452		4731 
...
performance of contour tree computation depends on the size of the dataset and the number of
critical points present in it, the contour generation and the descriptor computation stages
are primarily affected by the size and the number of contours processed. The contour tree
...
symmetry at multiple scales. The results of our algorithm on Vortex and EMDB-1179 datasets
are included in the supplemental figures.
\section{Conclusion}
