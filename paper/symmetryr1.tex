...
\teaser{
	...
	\subfigure[]
	{
		\begin{minipage}[c][1\width]{0.32\textwidth}
		{
			...
			\vspace{0.6cm}
%			\includegraphics[scale=.25]{figures/descspace.png}
		}
		\end{minipage}
	}	
	\subfigure[]
	...
	data of AMP-activated kinase (EMDB-1897). (a)~Volume rendering of the data displays different symmetric patterns
	that exhibit 3-fold rotational symmetry. (b)~Contours 
	are represented as points in a high-dimensional descriptor space (illustrated in 2D) using
	a suitable shape descriptor. Symmetric contours form a cluster in the descriptor space and can be easily identified.  
	Three such clusters located are shown in orange, blue, and pink colours. (c)~Three symmetric regions of
	different sizes highlighted in orange, blue, and pink colours demonstrates the multiscale aspect of our method.}
...
\begin{figure}[b]
	...
	\subfigure[]
	{
		\begin{minipage}[c][1\width]{0.3\linewidth}
		{
			...
			\includegraphics[scale=.25]{figures/partct.png}
		}
		\end{minipage}
	}
	\subfigure[]
...
pressure region extracted is shown in yellow. (e)-(h)~The isocontour from the low pressure region of the
first time step is employed as the query and the result on four different time steps, 12, 24, 36, and 48
...
\noindent{\textbf{Stabilization parameter $\alpha$}}.
...
too small a value for the stabilization parameter $\alpha$ may result in only partial detection
...
(EMDB-1292) with 7-fold rotational symmetry, which can be noticed from the volume rendering
in Figure~\ref{}. In an attempt to capture the symmetry of the small unstable regions within
the long tube-like regions that form the 7-fold symmetry, we set $\alpha$ to 0.5\% of 
the length of the total range of scalar values. However, only two of the seven symmetric
regions generates contours belonging to the small regions within the tube-like regions, shown
in red in Figure~\ref{}. Due to the instability of the saddles, the contours belonging to
the remaining five regions for the same isovalue have already merged as shown by the green contours.

To determine a suitable value for $\alpha$, we adopt the same heuristic used by Thomas and Natarajan
to stabilize the branch decomposition representation of the contour tree~\ref{ThomN11}. We plot
the drop in the number of branches with increasing simplification of the contour tree,
see Figure~\ref{}. Initially, there is a sudden drop due to the unstable critical points, 
after which the curve tapers off. Based on this plot, we select a value after the curve tapers off.
Setting $\alpha$ to 2\% discards the unstable isovalue generated earlier. 
Figure~\ref{} depicts the 7-fold symmetry of the small blue regions within the tube-like
regions identified using the new isovalue generated. The symmetry of the remaining small regions
are not detected since there are no stable contours in this region. We observe that the exact
value of $\alpha$ is crucial only while determining symmetry of the contours belonging to 
...
as illustrated in Figure~ref{}. This is because these contours belong to longer arcs and are 
insensitive to the exact value of $alpha$ as long as it is not too close to the saddle.
...
We choose a value immediately after this sudden drop and set this as the value of $\delta$. We
set $\delta=$20 for the fuel dataset and four sets of symmetric features, each exhibiting
4-fold rotational symmetry are identified, see Figure~\ref{fuel-sym}. Observe that the
the supplemental material of the extremum graph based symmetry detection method shows 
...
method is able to capture the symmetry of smaller symmetric regions for this dataset. They use a different model for determining
noise based on persistence, which is the difference in function value between the extremum
and the saddle of a topological feature. Although our implementation may be modified to incorporate
