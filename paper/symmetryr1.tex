...
%% Abstract section.
\abstract{The complex nature of volumetric data often limits the scope of direct exploration
of features in the data. Isocontours are quite popular in analysing the characteristics
of scalar field data due to its simpler presentation and assists users in segmenting
important features in the data via the contour tree. In this paper, we present a novel representation 
of contours in a scalar field with the aim of studying the similarity relationship between the 
contours. The representation maps contours as points in a high dimensional transformation invariant 
descriptor space. We leverage the power of this representation to design a clustering based algorithm for 
detecting symmetric regions in a scalar field. Symmetry detection is a challenging problem because it demands
both segmentation of the data and identification of transformation invariant segments.
While the former task can be addressed using topological analysis of scalar fields,
the latter requires geometry based solutions. Our approach combines the two by utilizing the contour
tree for segmenting the data and the descriptor space for identifying transformation invariance.
We show the effectiveness of our method in designing applications like query driven exploration using
contours and asymmetry visualization.
} % end of abstract
